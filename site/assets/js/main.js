...
function initGallery3D() {
    ...
    if (!imagePaths.length) {
        for (var n = 1; n <= 7; n++) imagePaths.push('/site/assets/img/screenshot-' + n + '.png');
    }
...
/* --------------------------------------------------------------------------
   CTA — la burbuja sigue al cursor
   -------------------------------------------------------------------------- */

 
document.addEventListener('DOMContentLoaded', () => {
  const cta  = document.querySelector('.cta');
  const blob = document.querySelector('.cta-blob--pointer');
  if (!cta || !blob) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (!window.matchMedia('(hover: hover)').matches) return;   // salta en táctil
  ...
  cta.addEventListener('pointermove', (e) => {
    ...
    requestAnimationFrame(() => {
      blob.style.setProperty('--cta-x', x + 'px');
      blob.style.setProperty('--cta-y', y + 'px');
      pending = false;
    });
  });
 
  cta.addEventListener('pointerleave', () => {
    blob.style.setProperty('--cta-x', '50%');
    blob.style.setProperty('--cta-y', '50%');
  });
});

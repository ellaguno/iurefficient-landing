...
(function () {
  ...
  CMS.site("motion/cursor", function () {
    ...
    // el cursor es de todo el sitio: sus ajustes se leen de la primera sección que lo declare
    var host = document.querySelector('[data-effect~="motion/cursor"]') || document.body;
    var o = CMS.fx(host, "motion/cursor"), pull = CMS.fxNum(o, "pull", 0.3, 0, 1);
    CMS.load("gsap").then(function () {
      ...
      if (o.size) c.style.setProperty("--mo-cursor-size", o.size);
      if (o.color) c.style.setProperty("--mo-cursor-color", o.color);
      ...
      document.querySelectorAll("a, button, [data-magnetic]").forEach(function (el) {
        ...
        if (el.hasAttribute("data-magnetic") && pull > 0) el.addEventListener("mousemove", function (e) { var r = el.getBoundingClientRect(); gsap.to(el, { x: (e.clientX - r.left - r.width / 2) * pull, y: (e.clientY - r.top - r.height / 2) * pull, duration: 0.3 }); });
      });
    });
  });
})();
